...
export default class MineEditMessagePage extends React.PureComponent {
  ...
  clickConfirmButton() {
    ...
    if (navigation.state.params.title === '昵称') {
      ...
      this.props.navigation.navigate('MineEditConfirmMobilePage', {
        title: '手机号码',
        type: '确认',
      ...
      if (this.secontInputText !== '') {
        if (CommonUtils.checkEmail(this.secontInputText) === false) {
          Toast.info('邮箱输入不正确');
          return;
        }
      ...
      >
        ...
        >
          <MineCommonEditLayout
            ...
            // 传递省份
            passProvinceFunc={(text, provinceId) => {
              this.province = text;
